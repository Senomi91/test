...
const BlogPage = () => {
  ...
  const getArticoli = async () => {
    try {
      ...
      console.log('RISPOSTA DA STRAPI:', data); // Log per verificare la risposta

      // Verifica se la risposta ha i dati che ci aspettiamo
      if (data && data.data) {
        setArticoli(data.data); // Salva i dati nel nostro stato
      } else {
        console.error('Dati non trovati nella risposta');
      }
    } catch (error) {
